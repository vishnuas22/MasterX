...
class RateLimitConfig:
    ...
    IP_REQUESTS_PER_MINUTE: int = int(os.getenv("IP_REQUESTS_PER_MINUTE", "60"))
    IP_REQUESTS_PER_HOUR: int = int(os.getenv("IP_REQUESTS_PER_HOUR", "1000"))
    
    # Per-user limits (fair usage)
    USER_REQUESTS_PER_MINUTE: int = int(os.getenv("USER_REQUESTS_PER_MINUTE", "30"))
    USER_REQUESTS_PER_HOUR: int = int(os.getenv("USER_REQUESTS_PER_HOUR", "500"))
    USER_REQUESTS_PER_DAY: int = int(os.getenv("USER_REQUESTS_PER_DAY", "5000"))
    
    # Per-endpoint limits (protect expensive operations)
    CHAT_REQUESTS_PER_MINUTE: int = int(os.getenv("CHAT_REQUESTS_PER_MINUTE", "10"))
    VOICE_REQUESTS_PER_MINUTE: int = int(os.getenv("VOICE_REQUESTS_PER_MINUTE", "5"))
    
    # Cost-based limits (prevent budget drain)
    USER_DAILY_COST_LIMIT: float = float(os.getenv("USER_DAILY_COST_LIMIT", "5.0"))
    GLOBAL_HOURLY_COST_LIMIT: float = float(os.getenv("GLOBAL_HOURLY_COST_LIMIT", "100.0"))
    
    # Anomaly detection thresholds
    ANOMALY_SCORE_THRESHOLD: float = float(os.getenv("ANOMALY_SCORE_THRESHOLD", "0.8"))
    SPIKE_MULTIPLIER: float = float(os.getenv("SPIKE_MULTIPLIER", "3.0"))
    
    # Storage settings
    WINDOW_SIZE_SECONDS: int = int(os.getenv("WINDOW_SIZE_SECONDS", "3600"))
    MAX_HISTORY_ITEMS: int = int(os.getenv("MAX_HISTORY_ITEMS", "10000"))
...
# ============================================================================
# REQUEST WINDOW (Sliding Window Algorithm)
# ============================================================================
...
class AnomalyDetector:
    ...
    def detect_anomaly(self, key: str, current_rate: float) -> Tuple[bool, float]:
        ...
        # Spike detection
        spike_ratio = current_rate / (baseline + 0.0001)
        is_spike = spike_ratio > RateLimitConfig.SPIKE_MULTIPLIER
        
        # Anomaly score (0.0 to 1.0)
        z_score_normalized = min(1.0, z_score / 3.0)
        spike_normalized = min(1.0, spike_ratio / 5.0)
        ...
        if is_anomaly:
            logger.warning(
                f"Anomaly detected for {key}: "
                f"rate={current_rate:.2f}, baseline={baseline:.2f}, score={anomaly_score:.2f}"
            )
